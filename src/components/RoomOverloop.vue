<template>
  ...
  >
    ...
    <!-- Achtergrond textuur - zolder thema -->
    <div
      class="absolute inset-0 bg-gradient-to-br from-orange-950 via-gray-900 to-black opacity-90"
    ></div>
    ...
    <div class="relative z-10 max-w-7xl w-full px-16">
      ...
      <h2
        class="text-6xl md:text-7xl font-bold text-orange-500 mb-12 text-center"
      >
        🎯 Overloop/Zolder
      </h2>
      ...
      <!-- Content container -->
      <div
        class="bg-gray-800 bg-opacity-50 rounded-2xl p-12 shadow-2xl border-2 border-orange-700/30"
      >
        <h3 class="text-4xl text-orange-400 mb-6 text-center font-serif">
          Het Sjoelspel der Generaties
        </h3>
        ...
        <!-- Code invoer -->
        <div
          class="bg-gray-900/50 rounded-xl p-8 mb-8 border border-orange-600/30"
        >
          <label class="block text-2xl text-orange-400 mb-4"
          ...
          <input
            ...
            inputmode="numeric"
            maxlength="4"
            class="w-full px-6 py-4 text-4xl text-center tracking-widest bg-gray-800 text-white rounded-xl border-2 border-orange-600/50 focus:border-orange-500 focus:outline-none"
            placeholder="_ _ _ _"
            @keyup.enter="checkCode"
          />
          <button
            @click="checkCode"
            class="w-full mt-6 px-8 py-4 bg-gradient-to-r from-orange-700 to-orange-600 text-white text-3xl font-bold rounded-xl hover:from-orange-600 hover:to-orange-500 transition-all transform hover:scale-105"
          >
            Controleer Code
          </button>
        </div>
...
// Puzzle data
const roomTitle = `Overloop Zolder - Het Sjoelspel der Generaties`;
const roomDescription = `Op zolder staat al generaties lang de oude sjoelbak van de familie. Een rijmpje hangt erbij aan de muur.`;

const solutionCode = `7641`; // De juiste code
const hints = [`Sjoelen maar!`, `Volgorde uit score`];
...
onMounted(() => {
  // Load saved progress
  const saved = localStorage.getItem("escapeRoomOverloop");
  if (saved) {
...
const checkCode = () => {
  ...
  if (inputCode.value === solutionCode || allowEveryCode) {
    // Correct!
    message.value = "✅ Correct! Het sjoelspel is opgelost!";
