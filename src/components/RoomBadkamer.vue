<template>
  ...
  >
    ...
    <!-- Achtergrond textuur - badkamer thema -->
    <div
      class="absolute inset-0 bg-gradient-to-br from-cyan-950 via-gray-900 to-black opacity-90"
    ></div>
    ...
    <div class="relative z-10 max-w-7xl w-full px-16">
      ...
      <h2
        class="text-6xl md:text-7xl font-bold text-cyan-500 mb-12 text-center"
      >
        🚿 De Badkamer
      </h2>
      ...
      <!-- Content container -->
      <div
        class="bg-gray-800 bg-opacity-50 rounded-2xl p-12 shadow-2xl border-2 border-cyan-700/30"
      >
        <h3 class="text-4xl text-cyan-400 mb-6 text-center font-serif">
          De Verdwenen Tegelzetter
        </h3>
        ...
        <!-- Code invoer -->
        <div
          class="bg-gray-900/50 rounded-xl p-8 mb-8 border border-cyan-600/30"
        >
          <label class="block text-2xl text-cyan-400 mb-4"
          ...
          <input
            ...
            inputmode="numeric"
            maxlength="4"
            class="w-full px-6 py-4 text-4xl text-center tracking-widest bg-gray-800 text-white rounded-xl border-2 border-cyan-600/50 focus:border-cyan-500 focus:outline-none"
            placeholder="_ _ _ _"
            @keyup.enter="checkCode"
          />
          <button
            @click="checkCode"
            class="w-full mt-6 px-8 py-4 bg-gradient-to-r from-cyan-700 to-cyan-600 text-white text-3xl font-bold rounded-xl hover:from-cyan-600 hover:to-cyan-500 transition-all transform hover:scale-105"
          >
            Controleer Code
          </button>
        </div>
...
// Puzzle data
const roomTitle = `De Badkamer - De Verdwenen Tegelzetter`;
const roomDescription = `Tijdens de renovatie van 1985 gebeurde er iets vreemds. Tegelzetter Johannes werkte weken aan deze badkamer. Op zijn laatste dag liet hij een briefje achter: De nieuwe tegels zijn prachtig, maar wat ik achter de oude vond... dat heb ik voor de zekerheid op vijf plekken verstopt. Johannes werd nooit meer gezien. Zoek de vijf briefjes die Johannes achterliet.`;

const solutionCode = `1900`; // De juiste code
const hints = [
  `Zoek overal in de badkamer`,
  `De briefjes vormen samen een opdracht`,
  `Tel de tegels volgens de instructie`,
];

// Zoeklocaties voor briefjes
const searchLocations = ref([
  { id: 1, name: "Achter de spiegel", searched: false, hasNote: true },
  { id: 2, name: "Onder de wastafel", searched: false, hasNote: true },
  { id: 3, name: "In het medicijnkastje", searched: false, hasNote: false },
  { id: 4, name: "Achter het toilet", searched: false, hasNote: true },
  { id: 5, name: "In de douchebak", searched: false, hasNote: true },
  { id: 6, name: "Boven de deur", searched: false, hasNote: false },
  { id: 7, name: "Achter de radiator", searched: false, hasNote: true },
  { id: 8, name: "Onder het bad", searched: false, hasNote: false },
  { id: 9, name: "In de ventilatierooster", searched: false, hasNote: false },
]);

const noteTexts = {
  1: "Tel alle zwarte tegels op de vloer",
  2: "en de witte tegels boven de wastafel",
  4: "plus de blauwe tegels in de douchehoek",
  5: "en vergeet de gebroken tegels achter het toilet niet",
  7: "Het totaal is je code",
};
...
// Computed property om te checken of alle briefjes gevonden zijn
const allNotesFound = computed(() => {
  return foundNotes.value.length === 5;
});

// Functie om briefje tekst op te halen
const getNoteText = (noteId) => {
  const locationId = searchLocations.value.find(
    (loc) =>
      loc.hasNote &&
      foundNotes.value.indexOf(noteId) ===
        searchLocations.value
          .filter((l) => l.hasNote && l.searched)
          .indexOf(loc)
  )?.id;
  return noteTexts[locationId] || "";
};
...
const searchLocation = (locationId) => {
  ...
  if (location.hasNote) {
    foundNotes.value.push(foundNotes.value.length + 1);
  }
};

onMounted(() => {
  // Load saved progress
  const saved = localStorage.getItem("escapeRoomBadkamer");
  if (saved) {
    ...
    foundNotes.value = data.foundNotes || [];
...
const checkCode = () => {
  ...
  if (inputCode.value === solutionCode || allowEveryCode) {
    // Correct!
    message.value = "✅ Correct! Je hebt het geheim van Johannes ontdekt!";
